...
public class WheelOptions<T> {
    ...
    public void setPicker(List<T> options1Items,
        ...
        if (mOptions3Items != null  && mOptions3Items.size() > 0) {
            if (mOptions3Items.get(0).size() == 0){
                return;
            }
        ...
        // 选项4
        if (mOptions4Items != null  && mOptions4Items.size() > 0) {
            if (mOptions4Items.get(0).get(0).size() == 0){
                return;
            }
        ...
        if (this.mOptions2Items == null  ||  mOptions2Items.size() < 1) {
            wv_option2.setVisibility(View.GONE);
        } else {
            wv_option2.setVisibility(View.VISIBLE);
        }
        if (this.mOptions3Items == null  || mOptions3Items.size() < 1) {
            wv_option3.setVisibility(View.GONE);
        } else {
            wv_option3.setVisibility(View.VISIBLE);
        }
        if (this.mOptions4Items == null || mOptions4Items.size() < 1) {
        ...
        wheelListener_option1 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                int opt2Select = 0;
                int opt3Select = 0;
                if (mOptions2Items == null || mOptions2Items.size() < 1) {//只有1级联动数据
                    ...
                    if (mOptions3Items == null) {//只有2级联动数据
                        if (optionsSelectChangeListener != null) {
                            optionsSelectChangeListener.onOptionsSelectChanged(index, opt2Select, 0, 0);
                        }
                    }else {
                        if (!isRestoreItem) {
                            opt3Select = wv_option3.getCurrentItem();//上一个opt3的选中位置
                            //新opt3的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                            opt3Select = opt3Select >= mOptions3Items.get(index).get(opt2Select).size() - 1 ? mOptions3Items.get(index).get(opt2Select).size() - 1 : opt3Select;
                        }
                        wv_option3.setAdapter(new ArrayWheelAdapter(mOptions3Items.get(index).get(opt2Select)));
                        wv_option3.setCurrentItem(opt3Select);
                        if (mOptions4Items != null) {
                            wheelListener_option3.onItemSelected(opt3Select);
                        } else {
                            if (optionsSelectChangeListener != null) {
                                optionsSelectChangeListener.onOptionsSelectChanged(index, opt2Select, opt3Select, 0);
                            }
                        }
                    }
        ...
        wheelListener_option2 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                if (mOptions3Items != null && mOptions3Items.size() > 0) {
                    ...
                    int opt4 = 0;
                    if (!isRestoreItem) {
                        Log.d("option3",mOptions3Items.toString());
                        // wv_option3.getCurrentItem() 上一个opt3的选中位置
                        //新opt3的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                        opt3 = wv_option3.getCurrentItem() >= mOptions3Items
                                .get(opt1Select)
                                .get(index).size() - 1 ?
                    ...
                    if (mOptions4Items == null ) {//3级联动数据实时回调
                        if (optionsSelectChangeListener != null) {
                            optionsSelectChangeListener.onOptionsSelectChanged(wv_option1.getCurrentItem(), index, opt3, 0);
                        }
                    }else {
                        Log.d("option4",mOptions4Items.toString());
                        if (!isRestoreItem) {
                            opt4 = wv_option4.getCurrentItem();//上一个opt3的选中位置
                            //新opt4的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                            opt4 = wv_option4.getCurrentItem() >= mOptions4Items
                                    .get(opt1Select)
                                    .get(index)
                                    .get(opt3).size() - 1 ? mOptions4Items.get(opt1Select).get(index).get(opt3).size() - 1 : wv_option4.getCurrentItem();
                        }
                        wv_option4.setAdapter(new ArrayWheelAdapter(mOptions4Items.get(opt1Select).get(index).get(opt3)));
        ...
        wheelListener_option3 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                if (mOptions4Items != null && mOptions4Items.size() > 0) {
                    ...
                    opt2Select = opt2Select >= mOptions4Items.size() - 1 ? mOptions4Items.size() - 1 : opt2Select;
                    index = index >= mOptions3Items.get(opt1Select).get(opt2Select).size() - 1 ? mOptions3Items.get(opt1Select).get(opt2Select).size() - 1 : index;
                    int opt4 = 0;
                    if (!isRestoreItem) {
                        // wv_option3.getCurrentItem() 上一个opt3的选中位置
                        //新opt3的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                        opt4 = wv_option4.getCurrentItem() >= mOptions4Items.get(opt1Select)
                                .get(opt2Select)
                                .get(index).size() - 1 ?
                                mOptions4Items.get(opt1Select).get(opt2Select).get(index).size() - 1 : wv_option4.getCurrentItem();
                    }
                    wv_option4.setAdapter(new ArrayWheelAdapter(mOptions4Items.get(wv_option1.getCurrentItem()).get(wv_option2.getCurrentItem()).get(index)));
                    wv_option4.setCurrentItem(opt4);

                    if (optionsSelectChangeListener != null) {
                        optionsSelectChangeListener.onOptionsSelectChanged(wv_option1.getCurrentItem(), wv_option2.getCurrentItem(), index, opt4);
                    }
                } else {
                    if (optionsSelectChangeListener != null) {
        ...
        // 添加联动监听
        if (options1Items != null && options1Items.size() > 0 && linkage) {
            wv_option1.setOnItemSelectedListener(wheelListener_option1);
        }
        if (options2Items != null && options2Items.size() > 0 && linkage) {
            wv_option2.setOnItemSelectedListener(wheelListener_option2);
        }
        if (options3Items != null && options3Items.size() > 0 && linkage) {
            wv_option3.setOnItemSelectedListener(wheelListener_option3);
        }
        if (options4Items != null && options4Items.size() > 0 && linkage && optionsSelectChangeListener != null) {
    ...
    public void setNPicker(List<T> options1Items, List<T> options2Items, List<T> options3Items, List<T> options4Items) {
        ...
        if (options2Items == null || options2Items.size() < 1) {
            wv_option2.setVisibility(View.GONE);
        } else {
            wv_option2.setVisibility(View.VISIBLE);
            if (optionsSelectChangeListener != null) {
        ...
        if (options3Items == null || options3Items.size() < 1) {
            wv_option3.setVisibility(View.GONE);
        } else {
            wv_option3.setVisibility(View.VISIBLE);
            if (optionsSelectChangeListener != null) {
        ...
        if (options4Items == null || options4Items.size() < 1) {
            wv_option4.setVisibility(View.GONE);
        } else {
            wv_option4.setVisibility(View.VISIBLE);
            if (optionsSelectChangeListener != null) {
     ...
     * 分别设置第一二三四级是否循环滚动
     *
     * @param cyclic1,cyclic2,cyclic3,cyclic4 是否循环
     */
